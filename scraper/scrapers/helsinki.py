"""
Scraper: City of Helsinki — procurement and tender notices (hel.fi).

Helsinki publishes procurement calls via its public website and the
Helsinki region procurement unit (Hankinnat.hel.fi).
URLs:
  https://www.hel.fi/en/decision-making/city-organization/procurements
  https://www.hankintapalvelut.hel.fi/  (procurement services portal)
"""
import hashlib
import logging
...
BASE   = "https://www.hel.fi"
PAGES  = [
    f"{BASE}/en/city-and-administration/strategy-and-economy/procurement",
    f"{BASE}/fi/kaupunki-ja-hallinto/strategia-ja-talous/hankinnat",
    "https://hankinnat.hel.fi/fi/kilpailutukset",
]


class HelsinkiScraper(BaseScraper):
    SOURCE_NAME  = "Helsinki"
    BASE_URL     = BASE
    POLITE_DELAY = 1.5
    ...
    def scrape(self) -> Iterator[dict]:
        ...
        for url in PAGES:
            try:
                ...
                logger.warning("Helsinki %s: %s", url, exc)
                continue

            items = (
                soup.select("article, li.procurement-item, div.procurement")
                or soup.select("table tbody tr")
                or soup.select("ul.listing li, div.list-item")
                or [
                    a.find_parent(["li", "div", "tr", "article"]) or a
                    for a in soup.find_all("a", href=True)
                    if any(
                        kw in (a.get_text() + a["href"]).lower()
                        for kw in ("hankinta", "procurement", "tender", "kilpailutus")
                    )
                ]
            )

            logger.info("Helsinki %s: %d items", url, len(items))
            for item in items:
                try:
                    ...
                    logger.warning("Helsinki item parse: %s", exc)
    ...
    def _parse(self, item, page_url: str) -> dict | None:
        ...
        full_text = f"{title} {desc} Helsinki Finland"
        ...
        rec.update({
            ...
            "title":                 title[:1000],
            "description":           desc,
            "contracting_authority": "City of Helsinki",
        ...
        rec["location_id"]      = self._location_id("Helsinki")
        return rec
